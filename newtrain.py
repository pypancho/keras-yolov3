...
from yolo3.newmodel import preprocess_true_boxes, preprocess_true_box, yolo_body, tiny_yolo_body, yolo_loss
...
def create_model(input_shape, anchors, num_classes, load_pretrained=False, freeze_body=2,
    ...
    image_input2 = Input(shape=( None, None, 3))
    h, w = input_shape
    ...
    model_body = yolo_body([image_input,image_input2], num_anchors//3, num_classes )
...
#    print('************',model_body.input[0].shape() )
#    print('model_body.shape**',model_body.input[0].shape(),'*y_true.shape', *y_true.shape(),'model_loss.shpe()', model_loss.shape())
    model = Model([model_body.input[0],model_body.input[1], *y_true], model_loss)
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes):
    ...
    while True:
        ...
        #image_data1 = [image_data, image_data]
        #image_data2 = np.array(image_data1)   # input of original yolo: image
        #print('image_data2', image_data2.shape)
        box_data = np.array(box_data) # output of original yolo: boxes

        y_true = preprocess_true_box(box_data, input_shape, anchors, num_classes) # some kind of output description?!
        yield [image_data,image_data, *y_true], np.zeros(batch_size)
       


    
def data_generator_wrapper(annotation_lines_rgb, batch_size, input_shape, anchors, num_classes):
    n = len(annotation_lines_rgb)
    if n==0 or batch_size<=0: return None
    return data_generator(annotation_lines_rgb, batch_size, input_shape, anchors, num_classes)
